...
contract ResumeVerification {
    ...
    function createResume() external {
        ...
        require(
            veriToken.erc20Contract().transferFrom(
                msg.sender,
                address(this),
                1
            ),
    ...
    function sendVerificationRequest(string memory content, address employer)
        external
        onlyEmployee
    {
    ...
    function updateVerificationRequestStatus(
        ...
        require(
            msg.sender == verificationRequests[requestId].employer,
        ...
        if (newStatus == Status.Verified) {
            ...
            Resume storage userResume = resumes[
                verificationRequests[requestId].employee
            ];
    ...
    function viewEmployeeResume(address employeeAddr)
        external
        onlyEmployer
    ...
    function getResumeItem(address _employee, address _employer)
        external
        view
        returns (ResumeEntry memory)
    {
        ...
        for (uint256 i = 0; i < resumes[_employee].entries.length; i++) {
            if (
                verificationRequests[resumes[_employee].entries[i].id]
                    .employer == _employer
            ) {
